...
public class GhostMotionController : MonoBehaviourPunCallbacks, IPunObservable //기존스크립트에서 건드린부분 MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [SerializeField] private Image RadioImg;
    [SerializeField] private Sprite RadioOn;
    [SerializeField] private Sprite RadioOff;
    ...
    void Update()
    {
        ...
        if (soundOnOff)
        {
            RadioImg.sprite = RadioOn;
        }
        else
        {
            RadioImg.sprite = RadioOff;
        }
    ...
    public void callReady(bool value)
    {
        photonView.RPC("PlayReady",RpcTarget.AllBuffered,value);
    }
    ...
    [PunRPC]
    public void PlayReady(bool ready)
    {
        Ready = ready;
    }
    ...
    void RayCast() {
        ...
        ray = camera.ViewportPointToRay(new Vector3(0.5f, 0.5f,3f));
        selection  = rayHit.transform;
        if(highlight !=null &&  rayHit.transform == null){
        ...
        if(Physics.Raycast (ray, out rayHit,MAX_DISTANCE,layerMask)){
            ...
            if(nowObject_clone == null &&selection != null && selection.gameObject.tag == "Key" && selection != highlight){ // 수정 부분
    ...
    void OnTriggerEnter(Collider other){
        ...
        if(other.CompareTag("Enemy") && this.gameObject.tag!="Hide" && PV.IsMine){
            this.gameObject.transform.position = new Vector3(-2.2f,20f,5f);
    ...
    private IEnumerator DelayAndActivate(){
        ...
        photonView.RPC("Deadtag",RpcTarget.AllBuffered,"Dead");
    }

    [PunRPC]
    public void Deadtag(string tag)
    {
